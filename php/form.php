<?php 
// Email settings 
$toEmail = 'kinggeorgetammy@gmail.com'; // Recipient email 
$from = 'info@kikuhhc.com'; // Sender email 
$fromName = 'Kiku HHC Website'; // Sender name 
...
if(isset($_POST['submit'])){ 
    ...
    if(empty($phone)){ 
        $valErr .= 'Please enter your cell number.<br/>'; 
    } 
    if(empty($message)){ 
    ...
    if(empty($valErr)){ 
        ...
        if(!empty($_FILES["resumefile"]["name"])){ 
            ...
            if(in_array($fileType, $allowFileTypes)){ 
                ...
                if(move_uploaded_file($_FILES["resumefile"]["tmp_name"], $targetFilePath)){ 
        ...
        if($uploadStatus == 1){ 
            // Email subject 
            $emailSubject = 'Application Submitted by '.$fname $lname; 
             
            // Email message  
            $htmlContent = '<h2>Application Submitted</h2> 
                <p><b>First Name:</b> '.$fname.'</p> 
                <p><b>Last Name:</b> '.$lname.'</p> 
                <p><b>Email:</b> '.$email.'</p> 
                <p><b>Cell Number:</b> '.$phone.'</p> 
